...
//
//        final BookTo bookTo1 = new BookTo(1L, bookTitle, new ArrayList<AuthorTo>(Arrays.asList(new AuthorTo("Name","Author1", 1L))));
//        final BookTo bookTo2 = new BookTo(2L, bookTitle, new ArrayList<AuthorTo>(Arrays.asList(new AuthorTo("Name","Author2", 2L))));
//
...
//                .andExpect(jsonPath("[1].title").value(bookTo2.getTitle()));
    ...
    @Test
    @Ignore
    public void testShouldEditBookTitle() throws Exception {
    	// given
    	File file = FileUtils.getFileFromClasspath("classpath:pl/spring/demo/web/json/bookToSave.json");
    	String json = FileUtils.readFileToString(file);
    	// when
    	ResultActions response = this.mockMvc.perform(get("/books/edit-book?bookId=1")
    			.accept(MediaType.APPLICATION_JSON)
    			.contentType(MediaType.APPLICATION_JSON)
    			.content("newTitle")
    			.content(json.getBytes()));
